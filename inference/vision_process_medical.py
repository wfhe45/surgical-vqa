...
def fetch_video(ele: Dict[str, Any], image_patch_size: int = 14, return_video_sample_fps: bool = False,
    ...
    if isinstance(ele["video"], str):
        ...
        # ===== MEDICAL-SPECIFIC: Apply RC box drawing if needed =====
        is_rc = ele.get("is_RC", False)
        rc_info = ele.get("RC_info", None)
        if is_rc and rc_info is not None:
            start_frame_path = rc_info.get("start_frame")
            bbox = rc_info.get("start_frame_bbox")
            video_list = list(ele["video"])
            if start_frame_path and bbox and start_frame_path in video_list:
                try:
                    from PIL import ImageDraw
                    idx = video_list.index(start_frame_path)
                    frame_img = image_list[idx]
                    # Handle portrait orientation (rotate if needed)
                    if frame_img.height > frame_img.width:
                        frame_img = frame_img.rotate(270, expand=True)
                    draw = ImageDraw.Draw(frame_img)
                    draw.rectangle(bbox, outline=(0, 255, 0), width=8)
                    image_list[idx] = frame_img
                except Exception as e:
                    logger.warning(f"Failed to draw RC box: {e}")
    ...
    max_pixels_dynamic = max(min(VIDEO_FRAME_MAX_PIXELS, total_pixels / nframes * FRAME_FACTOR), int(min_pixels * 1.05))
    max_pixels_supposed = ele.get("max_pixels", max_pixels_dynamic)


    if max_pixels_supposed > max_pixels_dynamic:
        logger.warning(f"The given max_pixels[{max_pixels_supposed}] exceeds limit[{max_pixels_dynamic}].")
    max_pixels = min(max_pixels_supposed, max_pixels_dynamic)
    ...
    if "resized_height" in ele and "resized_width" in ele:
        ...
        # print(f"  smart_resize result: {resized_height}x{resized_width}")
...
# Alias for VLLM inference compatibility
# The underlying fetch_video() already has all medical features:
# - RC box drawing (lines 433-452)
# - Per-sample FPS support (line 459)
process_vision_info_medical = process_vision_info